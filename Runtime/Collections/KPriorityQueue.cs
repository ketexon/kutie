...
// This is just a straight copy of:
// https://github.com/dotnet/runtime/blob/69d80765741a92af937f91c9c944a6b7ec56297b/src/libraries/System.Collections/src/System/Collections/Generic/PriorityQueue.cs
// So that we can use it in old .NET
// It removes the unorderedcollection and reverts some new features
// It also adds the Contains function
...
namespace Kutie.Collections
{
    ...
    /// </remarks>
    public class KPriorityQueue<TElement, TPriority>
    {
        /// <summary>
        ...
        /// </param>
        public KPriorityQueue(IComparer<TPriority> comparer)
        {
            _nodes = Array.Empty<(TElement, TPriority)>();
        ...
        ///  The specified <paramref name="initialCapacity"/> was negative.
        /// </exception>
        public KPriorityQueue(int initialCapacity, IComparer<TPriority> comparer)
        {
        ...
        /// </remarks>
        public KPriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items)
        ...
        ///  which is generally faster than enqueuing individual elements sequentially.
        /// </remarks>
        public KPriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority> comparer)
        {
			if(items == null){
				throw new ArgumentNullException(nameof(items));
			}

			if(items is ICollection<(TElement, TPriority)> collection){
				_nodes = new (TElement, TPriority)[collection.Count];
				collection.CopyTo(_nodes, 0);
				_size = collection.Count;
			}
			else {
				List<(TElement, TPriority)> list = new(items);
				_nodes = list.ToArray();
				_size = list.Count;
			}
            _comparer = InitializeComparer(comparer);
		...
		public bool Contains(TElement element, IEqualityComparer<TElement> equalityComparer = null) {
			int index = FindIndex(element, equalityComparer);
            return index >= 0;
		}
